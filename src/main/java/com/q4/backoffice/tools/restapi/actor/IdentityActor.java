...
import com.q4.backoffice.tools.restapi.config.AppConfig;
import com.q4.backoffice.tools.restapi.config.AppConstants;
...
public class IdentityActor extends AbstractActor {
	...
	public IdentityActor() {
		...
		query_SelectIdentities = AppConfig.getString(AppConstants.APP_NAME + ".dataSource.query.selectIdentities");
		query_SelectIdentity = AppConfig.getString(AppConstants.APP_NAME + ".dataSource.query.selectIdentity");

		//Load db Driver and url from Config
		String driverName = AppConfig.getString(AppConstants.APP_NAME + ".dataSource.className");
		String databaseUrl = AppConfig.getString(AppConstants.APP_NAME + ".dataSource.url");
	...
	private void processIdentityRequest(GetIdentity r) {
		...
		try {
			if (!permissions.checkedPermissionGranted(r.getUserid(), Permissions.PermissionItem.identities)) {
				...
				String result = dbQuery.Select(query_SelectIdentities, DbQuery.ResponseFormat.JSON, "identities", r.getIdentityUuid(), true);
				if (result.isEmpty()) {
